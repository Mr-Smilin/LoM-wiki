---
title: 隱藏參數列表
description: 隱藏參數列表
aside: false
outline: [2, 3]
sidebar: false
borderless: true
tags:
    - parameters
    - system
---

# {{ $frontmatter.title }}
::: warning
- 本條目處於草創階段。<br>
:::

## 簡介
- 遊戲中，有許多參數無法從遊戲介面直接觀看。
- 此表列出其中較泛用(非限於特定事件)，且數值並非單純「是/否」者，供讀者查閱。
- 與特定事件相關的參數，多為事件旗標🚩，請參見該事件條目。

## 參數列表
<Table>
  <tr>
    <th style="text-align: center">名稱</th>
    <th style="text-align: center">意義</th>
    <th style="text-align: center">作用</th>
    <th style="text-align: center">數值增減方式</th>
    <th style="text-align: center">備註</th>
  </tr>

  <tr>
    <td style="text-align: center">
      全武學點數      
    </td>
    <td>
      曾獲得的武學點數總和
    </td>
    <td>
      <li>影響劇情分歧，如<MarkdownWrapper>[師妹招親觀戰的點評](/event/simple/3-05-1-師妹招親後半)</MarkdownWrapper></li>
      <li><MarkdownWrapper>[增加單次修練最大消費點數](https://www.pttweb.cc/bbs/C_Chat/M.1722466578.A.B4D)</MarkdownWrapper>(最高25，全武學335時達到)</li>
      <li><MarkdownWrapper>[增加對決中的氣力上限](https://www.pttweb.cc/bbs/C_Chat/M.1785185363.A.B41)</MarkdownWrapper>(最高85，全武學350時達到)</li>
      <li>增加對決中看破對方意圖的機率(待確認)</li>
    </td>
    <td>
      <li>遊戲中任何可獲得武學點數的方式</li>
      <li>天命繼承系統加點武學</li>
    </td>
    <td>
      <li>數值可透過最大消費點數略估</li>
    </td>
  </tr>

  <tr>
    <td style="text-align: center">
      全煉丹點數
    </td>
    <td>
      曾獲得的煉丹點數總和
    </td>
    <td>
      <li>影響劇情分歧，如觸發<MarkdownWrapper>[治療大師兄](/event/detailed_description/4-02-2-飛俠歸來)</MarkdownWrapper></li>
      <li>項目的解鎖，如開發<MarkdownWrapper>[唐門牙粉](/system/facility)</MarkdownWrapper></li>
      <li><MarkdownWrapper>[增加單次煉丹最大消費點數](https://www.pttweb.cc/bbs/C_Chat/M.1722466578.A.B4D)</MarkdownWrapper>(最高25，全煉丹486時達到)</li>
    </td>
    <td>
      <li>遊戲中任何可獲得煉丹點數的方式</li>
      <li>天命繼承系統加點煉丹</li>
    </td>
    <td>
      <li>數值可透過最大消費點數略估</li>
    </td>
  </tr>

  <tr>
    <td style="text-align: center">
      全鍛造點數
    </td>
    <td>
      曾獲得的鍛造點數總和
    </td>
    <td>
      <li>影響劇情分歧，如<MarkdownWrapper>東西武林盟會戰後替夏侯蘭修傘</MarkdownWrapper></li>
      <li>項目的解鎖，如開發<MarkdownWrapper>[變重石鎖](/system/facility)</MarkdownWrapper></li>
      <li><MarkdownWrapper>[增加單次鍛造最大消費點數](https://www.pttweb.cc/bbs/C_Chat/M.1722466578.A.B4D)</MarkdownWrapper>(最高25，全鍛造495時達到)</li>
    </td>
    <td>
      <li>遊戲中任何可獲得鍛造點數的方式</li>
      <li>天命繼承系統加點鍛造</li>
    </td>
    <td>
      <li>數值可透過最大消費點數略估</li>
    </td>
  </tr>

  <tr>
    <td style="text-align: center">
      廚藝
    </td>
    <td>
      烹飪能力
    </td>
    <td>
      <li>影響劇情分歧，如<MarkdownWrapper>[此篇舉例](https://www.pttweb.cc/bbs/C_Chat/M.1722353758.A.CBE)</MarkdownWrapper></li>
      <li>影響<MarkdownWrapper>[伙房隨機事件機率](/system/training/)</MarkdownWrapper></li>
    </td>
    <td>
      <li>主要透過<MarkdownWrapper>[伙房工作指令](/system/training/)</MarkdownWrapper>增加</li>
      <li>其他指令的烹飪事件可增加，如後山修練烤魚</li>
      <li>劇情可獲得，如<MarkdownWrapper>[唐門財務改善](/event/simple/1-08-1-唐門財務改善)</MarkdownWrapper>中獲得廚具</li>
    </td>
    <td></td>
  </tr>

  <tr>
    <td style="text-align: center">
      變心點數
    </td>
    <td>
      對心上人的不忠誠度
    </td>
    <td>
      <li>達10點以上觸發變心夢，夢後歸零</li>
      <li>影響劇情分歧，如<MarkdownWrapper>[前往武林大會](/event/simple/3-09-1-前往武林大會)</MarkdownWrapper>時的對話</li>
    </td>
    <td>
      <li>主要透過劇情增減</li>
      <li>部份<MarkdownWrapper>[養成指令](/system/training/)</MarkdownWrapper>可增減，如男弟子房摺紙</li>
    </td>
    <td>
      <li>
          可透過增加多少變心點後觸發變<br>
          心夢，來推估當前數值(需先存檔)
      </li>
    </td>
  </tr>

  <tr>
    <td style="text-align: center">
      變心次數
    </td>
    <td>
      更改心上人的次數
    </td>
    <td>
      <li>影響劇情分歧，如<MarkdownWrapper>[大師兄對你的評價](/event/simple/3-01-3-圍觀臥雲崗)</MarkdownWrapper></li>
    </td>
    <td>
      <li>在變心夢中更換心上人時增加</li>
    </td>
    <td></td>
  </tr>

  <tr>
    <td style="text-align: center">
      門派好感
    </td>
    <td>
      各大勢力的好感度
    </td>
    <td>
      <li>影響劇情分歧，如<MarkdownWrapper>[武林大會](/event/simple/3-10-2-武林大會)</MarkdownWrapper>中各派投票的天命補正值</li>
    </td>
    <td>
      <li>劇情可增減，如<MarkdownWrapper>[前往武林大會](/event/simple/3-09-1-前往武林大會)</MarkdownWrapper>時泥教攔路事件</li>
      <li><MarkdownWrapper>[養成指令](/system/training/)</MarkdownWrapper>可增減，如唐門留學生事件</li>
    </td>
    <td></td>
  </tr>

</table>

## 備註
- 有數值但目前無作用的隱藏參數: 娘化、腿腳、槍術、軟兵器、魔攻、南宮淺黑化等等......。
