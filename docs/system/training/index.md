...
>
    ...
    <tr>
		...
		<td>
            ...
            <br>
            若🚩指導錦香弟子次數≥3，且未🚩觸發妒忌: 檢定武林大會同行者:<br>
            • 🚩郁竹同行武林大會: 觸發郁竹吃醋事件，設置🚩觸發妒忌<br>
            • 🚩虞小梅同行武林大會: 觸發虞小梅下藥事件，設置🚩觸發妒忌<br>
            • 其他: 無特殊劇情<br>
        </td>
		<td>-</td>
		<td>-</td>
...
- 「優先」事件:<br>
    ...
    3. 優先事件結果若寫接續該指令隨機事件，代表優先事件不會消耗行動點。<br>
    4. 反之，若未寫接續該指令隨機事件，代表優先事件結束時消耗行動點，行動回合結束。<br>
    5. 貢獻及部分數值增減，其實在優先事件中觸發。本表格式難以表達確切流程，僅盡量敘述等效結果。<br>
    6. 若僅單一優先事件(即「無優先事件」)，且無特別內容，本表略之不寫，其效果併入後續隨機事件效果中。<br>
