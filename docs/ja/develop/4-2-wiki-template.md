...
-   `<BadendIcon>` : <BadendIcon>生死簿 (バッドエンド)</BadendIcon>。クリックするとデフォルトで生死簿のメインページの対応位置にジャンプし、一部は詳細ページにジャンプします。
...
以下の利用可能なコンポーネントのいずれかで置き換えることができます。`no` とアイテム使用タイプが対応していれば大丈夫です。  
キャラクター Icon との違いは背景が追加されている点です。  
また、このコンポーネントが使用する画像は特定の位置に配置する必要があります。
...
HTML で囲まれたコンテンツ内の Markdown を解析するために使用されます。  
改行以外は `<br>` のみが使用でき、その他の Markdown はすべて使用できます。
...
4.  `searchMode`：検索タイプを設定します
    ...
    -   `and` <Badge type="warning" text="デフォルト値" /> ：AND検索
    -   `or`：OR検索
...
<InfoList position="left">
  ...
  <Info title="列伝二">
    ...
    一身の剣法はすべて家伝の秘笈から得たもので、龍淵七絶の中では特に剣法に巧みで、<br>
...
<EndBackground no=8 title="煮込まれた犬">
ウサギが死ぬと犬は煮られる。お前はもう用済みの手下だ。張家の<br>
饼ひとつで簡単に騙せてしまうなんて、本当に安い買い物だぜ。<br>
<br>
<br>
<!-- 註解：此行用於標示給編輯者使用，使用者是看不到的。用於提示此處有空行，因排版原因，請勿刪除 -->
</EndBackground>
...
1. エンディング説明は標記シンボルに直接配置されます。上記の例のように、`兔死狗烹,...可真划算。` というテキストの配置位置を参照してください。
...
5. テキストボックスの長さまたは幅が不足している場合は、自分で `<br>` 改行シンボルまたは「　」全角スペースを追加してください。  
   テキストの末尾に `<!-- レイアウトのため空行と全角スペースを挿入しており、理由なく削除しないでください -->` というコメントを追加してください。
